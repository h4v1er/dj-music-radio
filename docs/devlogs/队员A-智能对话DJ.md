...
- [x] 整合 `dev-user` 到当前 `dev-chat` 分支，修复用户注册登录、JWT、修改密码、收藏/历史联动和前端登录态刷新
- [x] 修复音乐中心歌单创建、右键添加到歌单、播放队列持久化、播放器布局、列表对齐、播放详情动画等集成问题
- [x] 修复推荐模块真实歌名补全、手动刷新今日推荐、情绪标签优先推荐和推荐面板交互样式
- [x] 新增答辩机一键启动/关闭脚本说明，更新最终项目文档、答辩项目文档和 PPT 制作稿
